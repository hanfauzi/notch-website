...
const Navbar = () => {
  ...
  const isActivePath = (path: string) => {
    ...
    if (normalizedTarget === "/projects") {
      return normalizedCurrent.startsWith("/projects");
    }
  ...
  const socmedLists = [
    {
      icon: <FaInstagram className="text-white text-2xl md:text-5xl" />,
      path: "https://instagram.com/",
    },
    {
      icon: <FaLinkedinIn className="text-white text-2xl md:text-5xl" />,
      path: "https://linkedin.com/",
    },
    {
      icon: <FaTiktok className="text-white text-2xl md:text-5xl" />,
  ...
  return (
    <header
      className={`z-50 text-black ${
        nav ? "bg-[#00000046] backdrop-blur-lg" : "bg-transparent"
      } fixed flex justify-between items-start md:items-end top-0 p-4 sm:px-7 md:px-14 md:py-8 lg:px-16 w-full duration-500`}
    >
      <Link href={"/"} className="w-fit">
        <Image
          src={notchLogo}
          alt="Notch Initial Logo"
          loading="eager"
          width={100}
          height={100}
      ...
      <nav className="hidden md:flex items-center">
        <ul className="flex gap-2 md:gap-6 items-center justify-center w-full py-2.5 text-white text-xs md:text-xl font-extralight tracking-wider">
          {navbarLists.map(({ name, path }, index) => (
            <li key={index}>
              <Link
                className={`pb-1 ${
                  isActivePath(path) && "font-semibold border-b-2 border-b-white"
                } hover:border-b-2 hover:border-b-white hover:text-white duration-100`}
                href={path}
              >
                {name}
              </Link>
              ...
              >
                {navbarLists.map(({ name, path }, index) => (
                  <ul
                    key={index}
                    className={`${
                      isActivePath(path) && "text-[#8d161e] translate-x-3"
                    } transform hover:translate-x-3 duration-300 overflow-hidden -my-1`}
                    onClick={() => setIsOpen(false)}
                  >
                    <m.li
                      variants={childMenu}
                      className={`text-4xl tracking-wide font-semibold py-1.5 text-start`}
                    >
              ...
              >
                {socmedLists.map(({ icon, path }, index) => (
                  <Link
                    key={index}
                    href={path}
                    className="hover:scale-90 duration-300 bg-[#8d161e] p-2 rounded-lg"
                  >
                    {icon}
                  </Link>
                ))}
